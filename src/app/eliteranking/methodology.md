**Abstract.** This paper describes the methodology behind the Yuhinkai artisan ranking system, which quantifies the stature of Japanese sword artisans through two independent Bayesian metrics derived from the official designation record. The *elite factor* models each artisan's rate of elite designation — Tokubetsu Juyo, Kokuho, Juyo Bunkazai, and equivalent tiers — as a Beta-Binomial posterior and extracts the 5th-percentile lower credible bound, penalizing small samples while converging to the observed rate for well-documented artisans. The *provenance factor* applies a Normal shrinkage estimator to the prestige scores of an artisan's documented former owners, regularizing thin provenance trails with conservative pseudoobservations at baseline. Both metrics are computed over a corpus of approximately 34,100 catalog records from seven source publications — including the NBTHK's Juyo and Tokubetsu Juyo zufu, government cultural property designations, the Imperial Collection, and the Jussi Ekholm koto research database — representing 25,472 unique physical objects after deduplication. The elite factor builds on Darcy Brockbank's pioneering pass factor, extending it with formal Bayesian uncertainty quantification. The two metrics are independent by design: an artisan may rank highly on one and modestly on the other, and the divergences are often the most informative readings in the system.

> *This is a working paper. Both the statistical methodology and the underlying data curation are active and ongoing. Rankings, counts, and methods described here are subject to revision as the dataset grows and the models are refined.*
