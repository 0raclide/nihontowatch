...
function BrowseContent() {
  ...
  const [activeTab, setActiveTab] = useState<'available' | 'sold'>(
    (searchParams.get('tab') as 'available' | 'sold') || 'available'
  );
  ...
  const handleTabChange = useCallback((tab: 'available' | 'sold') => {
    setActiveTab(tab);
    setPage(1);
  }, []);
            ...
            <div className="flex items-center">
              <button
                ...
                className={`px-4 py-1.5 text-[11px] uppercase tracking-[0.15em] transition-colors ${
                  activeTab === 'available'
                    ? 'text-ink dark:text-white border-b-2 border-gold'
                    : 'text-muted hover:text-charcoal dark:hover:text-gray-300 border-b-2 border-transparent'
                }`}
              >
                Available
              </button>
