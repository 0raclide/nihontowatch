...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: listing } = await (serviceClient.from('listings') as any)
      .select('id, dealer_id, source, koshirae')
      .eq('id', parsedItemId)
      .single() as { data: { id: number; dealer_id: number; source: string; koshirae: KoshiraeData | null } | null };
    ...
    // Upload to storage: {dealerId}/{listingId}/koshirae/{uuid}.{ext}
    const ext = file.type === 'image/png' ? 'png' : file.type === 'image/webp' ? 'webp' : 'jpg';
    const uuid = crypto.randomUUID();
    const path = `${auth.dealerId}/${itemId}/koshirae/${uuid}.${ext}`;
    ...
    // Append URL to koshirae images array
    ...
    const { error: updateError } = await (serviceClient.from('listings') as any)
      .update({ koshirae: updatedKoshirae })
      .eq('id', parsedItemId);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Remove URL from koshirae images array
    const koshirae = listing.koshirae;
    if (koshirae) {
      const updatedKoshirae: KoshiraeData = {
        ...koshirae,
      ...
      if (updateError) {
        logger.error('Failed to update koshirae after image removal', { error: updateError, listingId });
