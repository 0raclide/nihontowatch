 ...
 *
 * Searches the Yuhinkai artisan_makers table for smiths and tosogu makers.
 * Searches across: maker_id, name_romaji, name_romaji_normalized, name_kanji,
 * legacy_school_text, display_name
 *
 * Admin-only endpoint.
 */
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (queryError) {
      logger.error('Artisan search error', { error: queryError.message });
      return NextResponse.json(
