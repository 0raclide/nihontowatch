...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!kiwameId) return NextResponse.json({ error: 'kiwameId is required' }, { status: 400 });
    ...
    const { data: item } = await selectCollectionItemSingle(
      serviceClient, 'id', itemId, 'id, owner_id, item_uuid, kiwame'
    );
    ...
    const kiwameEntries = (item.kiwame as KiwameEntry[]) || [];
    ...
    const path = `${user.id}/${item.item_uuid}/kiwame/${uuid}.${ext}`;
    ...
    const { error: updateError } = await updateCollectionItem(serviceClient, itemId, { kiwame: updatedEntries });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const serviceClient = createServiceClient();
    const { data: item } = await selectCollectionItemSingle(
      serviceClient, 'id', itemId, 'id, owner_id, kiwame'
    );
    ...
    if (entryIndex !== -1) {
      ...
      const { error: updateError } = await updateCollectionItem(serviceClient, itemId, { kiwame: updatedEntries });
      if (updateError) {
        logger.error('Failed to update kiwame after image removal', { error: updateError, itemId });
