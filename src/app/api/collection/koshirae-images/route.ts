...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: item } = await selectCollectionItemSingle(
      serviceClient, 'id', itemId, 'id, owner_id, item_uuid, koshirae'
    );
    ...
    const koshirae: KoshiraeData = (item.koshirae as KoshiraeData) ?? {
      cert_type: null,
    ...
    const path = `${user.id}/${item.item_uuid}/koshirae/${uuid}.${ext}`;
    ...
    if (uploadError) {
      logger.error('Error uploading koshirae image', { error: uploadError });
      return NextResponse.json({ error: 'Failed to upload image' }, { status: 500 });
    }

    const { data: urlData } = serviceClient.storage.from(BUCKET).getPublicUrl(path);

    const updatedKoshirae: KoshiraeData = {
      ...koshirae,
      images: [...(koshirae.images || []), urlData.publicUrl],
    };

    const { error: updateError } = await updateCollectionItem(serviceClient, itemId, { koshirae: updatedKoshirae });

    if (updateError) {
      logger.error('Error updating koshirae images', { error: updateError });
    ...
    logger.logError('Koshirae image upload error', error);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const serviceClient = createServiceClient();
    const { data: item } = await selectCollectionItemSingle(
      serviceClient, 'id', itemId, 'id, owner_id, koshirae'
    );
    ...
    if (storageError) {
      logger.warn('Failed to remove koshirae image from storage', { error: storageError });
    }

    const koshirae = item.koshirae as KoshiraeData | null;
    if (koshirae) {
      const updatedKoshirae: KoshiraeData = {
        ...koshirae,
        images: (koshirae.images || []).filter((img: string) => img !== imageUrl),
      };

      const { error: updateError } = await updateCollectionItem(serviceClient, itemId, { koshirae: updatedKoshirae });
      if (updateError) {
        logger.error('Failed to update koshirae after image removal', { error: updateError, itemId });
    ...
    logger.logError('Koshirae image delete error', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
