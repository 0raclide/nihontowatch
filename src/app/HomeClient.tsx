...
export default function HomeContent() {
  ...
  const currentSearchIdRef = useRef<string | undefined>(undefined); // Correlation ID for CTR tracking
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (activity && queryKey && queryKey !== lastTrackedSearchRef.current) {
          ...
          const correlationId = activity.trackSearchQuery(
            queryKey,
            searchFilters,
            json.total || 0
          );
          currentSearchIdRef.current = correlationId;
        } else if (!queryKey) {
