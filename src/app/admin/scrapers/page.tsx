  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <MetricCard
          title="Images"
          value={`${stats?.images?.rate || 0}%`}
          subtitle={[
            `${stats?.images?.completed?.toLocaleString() || 0}/${stats?.images?.total?.toLocaleString() || 0} complete`,
            stats?.images?.partial ? `${stats.images.partial} partial` : '',
            stats?.images?.pending ? `${stats.images.pending} pending` : '',
            stats?.images?.failed ? `${stats.images.failed} failed` : '',
          ].filter(Boolean).join(' · ')}
          color={(stats?.images?.rate || 0) >= 95 ? 'green' : (stats?.images?.rate || 0) >= 80 ? 'yellow' : 'red'}
          icon={
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
