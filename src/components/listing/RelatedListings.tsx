...
function RelatedRow({ title, browseUrl, items }: {
  title: string;
  browseUrl: string;
  items: RelatedItem[];
}) {
  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-[13px] uppercase tracking-wider text-muted font-medium">
          {title}
        </h2>
        <Link
          href={browseUrl}
...
export function RelatedListings({
  ...
  const [artisanItems, setArtisanItems] = useState<RelatedItem[]>([]);
  const [dealerItems, setDealerItems] = useState<RelatedItem[]>([]);

  useEffect(() => {
    const fetchRelated = async () => {
      // Fetch artisan-related items
      if (artisanId) {
        try {
          const res = await fetch(`/api/browse?artisanCode=${artisanId}&limit=5&sort=recent`);
          if (res.ok) {
            const data = await res.json();
            const filtered = (data.listings || [])
              .filter((l: RelatedItem) => l.id !== listingId)
              .slice(0, 4);
            setArtisanItems(filtered);
          }
        } catch { /* silent */ }
      }

      // Fetch dealer-related items (same type if possible)
      try {
        const typeParam = itemType ? `&type=${itemType}` : '';
        const res = await fetch(`/api/browse?dealer=${dealerId}${typeParam}&limit=5&sort=recent`);
        if (res.ok) {
          const data = await res.json();
          const filtered = (data.listings || [])
            .filter((l: RelatedItem) => l.id !== listingId)
            .slice(0, 4);
          setDealerItems(filtered);
        }
      } catch { /* silent */ }
    };

    fetchRelated();
  }, [listingId, dealerId, artisanId, itemType]);
