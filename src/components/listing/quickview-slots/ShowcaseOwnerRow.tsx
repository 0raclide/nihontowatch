'use client';

import { useLocale } from '@/i18n/LocaleContext';
import type { ShowcaseExtension } from '@/types/displayItem';

interface ShowcaseOwnerRowProps {
  showcase?: ShowcaseExtension | null;
}

export function ShowcaseOwnerRow({ showcase }: ShowcaseOwnerRowProps) {
  const { t } = useLocale();
  if (!showcase) return null;

  const displayName = showcase.owner_display_name || t('showcase.fromCollector');

  return (
    <div className="flex items-center gap-2 text-sm text-muted">
      {showcase.owner_avatar_url ? (
        <img
          src={showcase.owner_avatar_url}
          alt=""
          className="w-5 h-5 rounded-full object-cover"
        />
      ) : (
        <svg className="w-4 h-4 text-muted/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
        </svg>
      )}
      <span>{t('showcase.ownerCollection', { name: displayName })}</span>
    </div>
  );
}
