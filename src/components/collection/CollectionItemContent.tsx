      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <div className="flex items-center mt-1.5 text-[12px] text-muted">
            {item.acquired_from ? (
              <>
                ...
                <span className="truncate">{t('collection.from')} {item.acquired_from}</span>
              </>
            ) : (
              <span className="text-muted/60">{t('collection.personalCollection')}</span>
            )}
          </div>
        </div>
        ...
        <div className="px-4 py-3 border-b border-border">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-3">
            ...
            {item.school && !displayName && (
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted block mb-0.5">{t('collection.school')}</span>
            ...
            {certInfo && (
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted block mb-0.5">{t('collection.papers')}</span>
                <span className="text-[13px] text-ink font-medium">
                  {certInfo.label}
        ...
        <div className="px-4 py-3 border-b border-border">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[13px]">
            {item.acquired_from && (
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted block mb-0.5">{t('collection.acquiredFrom')}</span>
                <span className="text-ink font-medium">{item.acquired_from}</span>
              </div>
            )}
            {acquiredDate && (
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted block mb-0.5">{t('collection.date')}</span>
                <span className="text-ink font-medium">{acquiredDate}</span>
              </div>
            )}
            {pricePaid && (
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted block mb-0.5">{t('collection.pricePaid')}</span>
                <span className="text-ink font-medium">{pricePaid}</span>
              </div>
            )}
            {currentValue && (
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted block mb-0.5">{t('collection.currentValue')}</span>
                <span className="text-ink font-medium">{currentValue}</span>
              </div>
            )}
            <div>
              <span className="text-[10px] uppercase tracking-wider text-muted block mb-0.5">{t('collection.status')}</span>
              <span className="text-ink font-medium">{statusLabel}</span>
            </div>
            {conditionLabel && (
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted block mb-0.5">{t('collection.condition')}</span>
        ...
        {item.catalog_reference && (
          <div className="px-4 py-3 border-b border-border">
            <h3 className="text-[10px] uppercase tracking-[0.1em] font-semibold text-muted mb-2">{t('collection.catalogRef')}</h3>
            <p className="text-[13px] text-ink">
              {item.catalog_reference.collection} {t('collection.vol')} {item.catalog_reference.volume}, #{item.catalog_reference.item_number}
            </p>
          </div>
        )}
      </div>
