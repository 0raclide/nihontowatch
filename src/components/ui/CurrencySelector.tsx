...
export function CurrencySelector({ value, onChange }: CurrencySelectorProps) {
  return (
    <div className="inline-flex items-center bg-linen/50 theme-dark:bg-gray-800/50 p-0.5 rounded-sm">
      {(Object.keys(CURRENCY_CONFIG) as Currency[]).map((currency) => (
        <button
          ...
          className={`px-2.5 py-1 text-[10px] tracking-wider transition-all duration-200 ${
            value === currency
              ? 'bg-white theme-dark:bg-gray-700 text-ink theme-dark:text-white shadow-sm'
              : 'text-muted theme-dark:text-gray-500 hover:text-charcoal theme-dark:hover:text-gray-300'
          }`}
        >
