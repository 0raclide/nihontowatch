...
import type { CuratorNoteContext, DataRichness } from './curatorNote';
...
const SYSTEM_PROMPT_EN = `You are a senior NBTHK-trained scholar writing museum wall text for a Japanese sword exhibition. Your voice matches the "Explanation" paragraphs in NBTHK Juyo and Tokubetsu Juyo designations — institutional, elevated, precise, never casual or promotional.
...
4. Designation counts ARE live data — cite specific statistics (e.g., "Among Sadamune's 47 designated works, only 12 hold Tokubetsu Juyo status").
...
- Paragraph 2 (Observation): Quote the setsumei's technical descriptions. Connect sayagaki observations. Highlight specific qualities the NBTHK identified.
...
ADAPTIVE LENGTH:
- Full data (setsumei + sayagaki/provenance + artisan): 300-400 words, 3 paragraphs.
- Moderate data (setsumei + artisan, no sayagaki/provenance): 200-300 words, 2-3 paragraphs.
...
- Use macrons on long vowels: Juyo, Tokubetsu Juyo, Soshu, Nanbokucho
...
function buildArtisanSection(
  ...
  if (designations.length > 0) {
    ...
    lines.push(`${lang === 'ja' ? '指定係数' : 'Designation factor'}: ${artisan.designation_factor.toFixed(2)}`);
  }
...
function buildEnInstruction(richness: DataRichness): string {
  switch (richness) {
    case 'full':
      return '[INSTRUCTION]\nWrite a 3-paragraph curator\'s note (300-400 words) following the Context → Observation → Significance arc. Quote the setsumei and sayagaki directly. Cite specific designation statistics. Output prose paragraphs only — no headers or bullets.';
    case 'moderate':
...
function buildJaInstruction(richness: DataRichness): string {
  switch (richness) {
    case 'full':
      return '【指示】\n文脈→観察→意義の3段落構成（800-1200文字）で解説文を執筆してください。説明書・鞘書を直接引用し、指定統計を具体的に記載してください。散文形式のみ — 見出し・箇条書き不可。';
    case 'moderate':
