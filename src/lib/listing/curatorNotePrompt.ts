...
4. Designation counts and artist statistical distributions ARE live data — cite specific statistics (e.g., "Among Sadamune's 47 designated works, only 12 hold Tokubetsu Jūyō status").
5. Never present expert judgment as your own conclusion.
6. Research notes from collectors/dealers are third-party claims. Attribute with "according to the consignor," "the owner notes," or similar hedging language. Never present unverified claims as established fact.
...
4. 指定統計および作者の統計的分布は具体的に引用してよい（例：「貞宗の指定作品47口中、特別重要は12口のみ」）。
5. 専門家の判断を自身の結論として提示しない。
6. 所蔵者・出品者からの調査ノートは第三者の主張である。「出品者によれば」「所蔵者は〜と述べている」等の表現で帰属を明示すること。未検証の主張を確定的事実として提示しない。
...
function buildResearchNotesSection(
  ...
  const label = lang === 'ja'
    ? '【調査ノート（出品者・所蔵者提供）】'
    : '[RESEARCH NOTES (Collector/Dealer Provided)]';
  const caveat = lang === 'ja'
    ? '※以下は出品者・所蔵者からの情報であり、独立した検証は行われていません。'
    : 'Note: The following is unverified information provided by the consignor/dealer.';
  return `${label}\n${caveat}\n${notes}`;
}
...
function buildArtistOverviewSection(
  ...
  // Elite ranking percentile
  if (overview.elite_percentile > 0) {
    lines.push(
      `${lang === 'ja' ? '指定ランキング' : 'Elite ranking'}: ${lang === 'ja' ? '上位' : 'Top '}${(100 - overview.elite_percentile).toFixed(1)}%`
    );
  }
