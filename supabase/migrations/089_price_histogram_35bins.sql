-- Migration: Update price histogram RPC to 35 bins with sub-¥100K range
-- Previous version (063) started at ¥100K — all cheap items crammed into bucket 0
-- Now includes 5 bins below ¥100K: 10K, 20K, 35K, 50K, 75K
...
DECLARE
  result jsonb;
  -- 35 log-spaced boundaries (JPY) — includes sub-¥100K range
...
BEGIN
  ...
  -- Assign each priced item to a bucket index (0-34)
  -- width_bucket(value, array) returns 1..N+1, so subtract 1 for 0-based
  -- and clamp to [0, 34] range
  bucketed AS (
    SELECT LEAST(GREATEST(width_bucket(price_jpy, v_boundaries) - 1, 0), 34) AS bucket_idx
    FROM filtered
  ),
