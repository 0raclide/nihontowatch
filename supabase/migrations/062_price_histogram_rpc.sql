-- Migration: Price histogram RPC for Airbnb/Zillow-style range slider
-- Returns log-spaced bucket counts for price distribution visualization
--
-- Cross-filtered by all dimensions EXCEPT priceMin/priceMax
-- (standard cross-filter pattern: histogram always shows full price distribution)
...
DECLARE
  result jsonb;
  -- 20 log-spaced boundaries (JPY)
  v_boundaries int[] := ARRAY[
    100000, 150000, 200000, 300000, 400000, 500000, 700000,
    1000000, 1500000, 2000000, 3000000, 4000000, 5000000,
    7000000, 10000000, 15000000, 20000000, 30000000, 45000000, 60000000
  ];
...
BEGIN
  ...
  -- Assign each priced item to a bucket index (0-19)
  -- width_bucket(value, array) returns 1..N+1, so subtract 1 for 0-based
  -- and clamp to [0, 19] range
  bucketed AS (
    SELECT LEAST(GREATEST(width_bucket(price_jpy, v_boundaries) - 1, 0), 19) AS bucket_idx
    FROM filtered
  ),
