...
The `buildBrowseUrl()` in `categoryPage.ts` includes `tab=available` so "Browse all" CTAs on category landing pages correctly bypass the redirect and open the full browse UI. It also maps `era` → `period` in the output URL, since the browse page's `useBrowseURLSync` reads `sp.get('period')` not `sp.get('era')`. Without this mapping, era-based "Browse all" links (koto-katana, shinto-katana, shinshinto-katana) would land on the browse page without the era filter applied.
